...
public class EarthTunneler extends EarthAbility {
	...
	public EarthTunneler(Player player) {
		super(player);
	...
	public static void revertAirBlocks() {
		if (ConfigManager.defaultConfig.get().getBoolean("Abilities.Earth.EarthTunnel.Revert")) {
			for (TempBlock tempBlock : EarthTunneler.airBlocks.keySet()) {
				if (EarthTunneler.airBlocks.get(tempBlock) + ConfigManager.defaultConfig.get().getLong("Properties.Earth.RevertCheckTime") <= System.currentTimeMillis()) {
					tempBlock.revertBlock();
					EarthTunneler.airBlocks.remove(tempBlock);
				}
			}
		}
	}
